...
OUT_JSON = Path("ai/data/eval_trace_time200.json")
OUT_CSV = Path("ai/data/eval_trace_time200.csv")
SLEEP_BETWEEN = 0.05
...
def evaluate():
    ...
    for idx, move in enumerate(game.mainline_moves(), 1):
        ...
        r = requests.post(
            ENGINE_URL, json={"fen": fen_after, "movetime": 200}, timeout=30
        )
        r.raise_for_status()
        data = r.json()
        ...
        print(
            ...
            f"| engine_reply: {engine_reply} | time: {used.get('elapsed_ms')} ms | depth≈{used.get('approx_depth')}"
        )
        ...
        trace["plies"].append(
            {
                ...
                "approx_depth": used.get("approx_depth"),
            }
        )
    ...
    print("=" * 70)
    print("Finished evaluating complete PGN at movetime=200ms.")
